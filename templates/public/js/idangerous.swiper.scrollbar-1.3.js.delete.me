...
Swiper.prototype.plugins.scrollbar = function(swiper, params){
	...
	if (window.HTMLElement) {
		...
		var container = (params.container instanceof Element) ? params.container : dq(params.container)[0]
	}
	...
	if (params.draggable) {
		...
		function setDragPosition(e){
			...
			var position;
			if (isH) {
				x = (e.pageX || e.clientX) - getOffset(track).left -dragWidth/2
				if (x<0) x = 0;
				...
				y = (e.pageY||e.clientY) - getOffset(track).top -dragHeight/2;
